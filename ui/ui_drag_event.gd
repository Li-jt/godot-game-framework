...
## 内部回调：由 UIDragManager 在创建 event 时设置。用于将 ghost 附着到场景树。
var _attach_ghost_cb: Callable


## [L2] 显示图标跟随鼠标。游戏层在 on_begin_drag 中调用。
## 返回 UIDragGhost 供游戏层进一步定制（如设置数量文本）。
func show_ghost_texture(p_texture: Texture2D, p_offset: Vector2 = Vector2(-24, -24)) -> UIDragGhost:
	var ghost := UIDragGhost.new()
...
func show_ghost_item(p_texture: Texture2D, p_count: int, p_offset: Vector2 = Vector2(-24, -24)) -> UIDragGhost:
	var ghost := UIDragGhost.new()
...
func show_ghost_text(p_text: String) -> UIDragGhost:
	var ghost := UIDragGhost.new()
...
func _attach(p_ghost: UIDragGhost) -> void:
