## GF_DefJsonLoader — 通用的 JSON 定义文件加载工具。
## 支持多个文件合并加载，后加载的覆盖同名字段。
## 供 UI/Audio/Input 等模块从 JSON 加载定义数据。
class_name GF_DefJsonLoader
extends RefCounted


## 加载一个 JSON 定义文件。返回 Dictionary，加载失败返回空字典。
static func load_file(p_fs: GF_FileSystemService, p_path: String) -> Dictionary:
	...
	if err != OK:
		push_error("[GF_DefJsonLoader] 解析 %s 失败: %s" % [p_path, json.get_error_message()])
		return {}
	return json.data


## 加载多个 JSON 文件并深度合并。p_paths 中靠后的覆盖靠前的。
static func load_and_merge(p_fs: GF_FileSystemService, p_paths: Array[String]) -> Dictionary:
	var result := {}
	for path in p_paths:
