...
> **ECS 定位提醒**：当前 ECS 是代码组织工具，不是性能工具。组件数据为 Dictionary/Variant，与 C++/Rust 连续内存 ECS 存在数量级差距。本文的优化建议是**在 GDScript 范围内减少浪费**，无法消除解释器和装箱开销。若实体达万级且 ECS 成为瓶颈，请走 [README 中的性能升级路径](../../../README.md#ecs-的定位代码组织工具不是性能工具)。
