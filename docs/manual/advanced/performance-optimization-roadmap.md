...
阶段 A（收尾，main 分支提交，短期）
  §1.1 零分配遍历提交 ──► §4 性能观测（计时 API + 面板壳）
  §1.2 Row 池 / §1.3 组件池（以 §4 数据为形态决策依据；
    §1.3 上线先决条件：使用方组件缓存已撤除，见 §1.3）
...
- **变更日志（§1.4）是阶段 B 的地基**——增量存档、区域管理、渲染脏标记、
  模拟免扫描全部依赖它；
- **观测先行**：§4 计时 API 是池化（§1.2/§1.3）的形态决策依据——池化是
  阶段 A 唯一破坏 API 生命周期的变更（Row 别名、组件引用失效），用框架自己的
  观测数据而非一次性实测投票；
...
| 阶段 | 验收 |
|------|------|
| A | 使用方（survival）GC 尖峰消除（分配计数曲线平直）；性能面板可显示子系统耗时；组件池上线前使用方组件缓存已撤除（§1.3） |
