...
class_name GF_ThreadJobOptions
extends RefCounted

## 任务名称（用于日志与调试）。
var name: String = ""
## 任务优先级（使用 GF_ThreadJobPriority.Level）。
var priority: int = GF_ThreadJobPriority.Level.NORMAL
## 任务标签（用于批量取消/统计分组）。
var tag: String = ""
## 超时时间（毫秒）。<= 0 时使用 GF_ThreadingService 默认值。
...
## 回调集合（全部在主线程触发）。
var callbacks: GF_ThreadJobCallbacks = GF_ThreadJobCallbacks.new()
