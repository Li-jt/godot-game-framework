...
##   [/codeblock]
class_name GF_AssetLoadingService
...
## 加载 PackedScene
func load_scene(p_path: String) -> GF_OperationResult:
	return _load(p_path, "PackedScene")


## 加载 Texture2D（贴图、精灵）
func load_texture(p_path: String) -> GF_OperationResult:
	return _load(p_path, "Texture2D")


## 加载 AudioStream（BGM、SFX）
func load_audio(p_path: String) -> GF_OperationResult:
...
func _load(p_path: String, p_type_label: String) -> GF_OperationResult:
	...
	return GF_OperationResult.ok(res)
