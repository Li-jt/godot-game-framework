## GF_ISaveable — 可存档模块基类（Framework 层）。
## 任何需要持久化的模块继承此类，重写 save_key() / on_save() / on_load()。
##
## 注意：不再自动注册。需通过 GF_SaveService.collect_from() 或 register_saveable() 显式注册。
