...
## 仅供 GF_AppBootstrap 装配阶段和 Framework 内部使用。
## Game 层禁止直接访问，应通过注入的 context（GF_GameServices）获取所需服务。
##
## 支持：
##   - 优先级覆盖：高优先级可覆盖低优先级同名服务
##   - 必需 key 声明：各 Installer 自声明自己产出的必需服务
##   - Mod 卸载：按 owner 批量注销
class_name GF_ServiceRegistry
extends RefCounted

# 标准服务 Key 常量
const KEY_AUDIO_RUNTIME: String = "GF_AudioRuntime"
const KEY_INPUT_ADAPTER: String = "GF_InputAdapter"
const KEY_SCHEDULER: String = "GF_Scheduler"
const KEY_SCENE_HOST: String = "GF_SceneHost"
const KEY_SCENE_FACTORY: String = "GF_SceneFactory"
...
const KEY_ECS_WORLD: String = "GF_EcsWorld"
const KEY_ECS_SCHEDULER: String = "GF_EcsScheduler"
const KEY_ECS_DEBUG: String = "EcsDebug"
const KEY_ECS_SAVE_ADAPTER: String = "GF_EcsSaveAdapter"
...
func register_with_priority(p_key: String, p_service: Object, p_owner: String = "system", p_priority: int = 100) -> GF_OperationResult:
	if p_key.is_empty():
		return GF_OperationResult.fail(GF_OperationResult.ERR_BAD_REQUEST, "注册失败: key 不能为空", "GF_ServiceRegistry")
	if p_service == null:
		return GF_OperationResult.fail(GF_OperationResult.ERR_BAD_REQUEST, "注册失败: service 不能为 null (key=%s)" % p_key, "GF_ServiceRegistry")
	...
	if _services.has(p_key):
		...
		if p_priority >= existing_priority:
			return GF_OperationResult.fail(
				GF_OperationResult.ERR_CONFLICT, "GF_ServiceRegistry",
				"服务 '%s' 已被 '%s' (priority=%d) 注册, 无法被 '%s' (priority=%d) 覆盖" % [p_key, existing_owner, existing_priority, p_owner, p_priority]
			)
		push_warning("[GF_ServiceRegistry] 服务 '%s' 被 '%s' 覆盖 (原有: '%s')" % [p_key, p_owner, existing_owner])

	if p_service is GF_ModuleLifecycle:
		var ml := p_service as GF_ModuleLifecycle
		if not ml.is_ready():
			return GF_OperationResult.fail(GF_OperationResult.ERR_PRECONDITION, "注册失败: 模块未 ready (key=%s, state=%d)" % [p_key, ml.state], "GF_ServiceRegistry")
...
func unregister(p_key: String, p_owner: String = "") -> GF_OperationResult:
	if not _services.has(p_key):
		return GF_OperationResult.fail(GF_OperationResult.ERR_NOT_FOUND, "GF_ServiceRegistry", "服务 '%s' 不存在" % p_key)
	if not p_owner.is_empty() and _owners.get(p_key, "") != p_owner:
		return GF_OperationResult.fail(GF_OperationResult.ERR_FORBIDDEN, "GF_ServiceRegistry", "'%s' 无权卸载 '%s' 的服务" % [p_owner, p_key])
...
func verify_pending() -> GF_OperationResult:
	...
	for key in _pending_required:
		...
		if svc is GF_ModuleLifecycle:
			var ml := svc as GF_ModuleLifecycle
			if not ml.is_ready():
...
func get_scene_host() -> GF_SceneHost: return _services.get(KEY_SCENE_HOST, null) as GF_SceneHost
func get_scene_factory() -> GF_SceneFactory: return _services.get(KEY_SCENE_FACTORY, null) as GF_SceneFactory
func get_asset_loading() -> GF_AssetLoadingService: return _services.get(KEY_ASSET_LOADING, null) as GF_AssetLoadingService
func get_threading(): return _services.get(KEY_THREADING, null)
func get_file_system() -> GF_FileSystemService: return _services.get(KEY_FILE_SYSTEM, null) as GF_FileSystemService
func get_path_resolver() -> GF_PathResolver: return _services.get(KEY_PATH_RESOLVER, null) as GF_PathResolver
func get_audio_service() -> GF_AudioService: return _services.get(KEY_AUDIO, null) as GF_AudioService
func get_input_service() -> GF_InputService: return _services.get(KEY_INPUT, null) as GF_InputService
func get_config_service() -> GF_ConfigService: return _services.get(KEY_CONFIG_SERVICE, null) as GF_ConfigService
func get_runtime_service() -> GF_RuntimeService: return _services.get(KEY_RUNTIME, null) as GF_RuntimeService
func get_save_service() -> GF_SaveService: return _services.get(KEY_SAVE, null) as GF_SaveService
func get_ui_service() -> GF_UIService: return _services.get(KEY_UI, null) as GF_UIService
func get_resource() -> GF_ResourceService: return _services.get(KEY_RESOURCE, null) as GF_ResourceService
func get_app_flow() -> GF_AppFlow: return _services.get(KEY_FLOW, null) as GF_AppFlow
func get_event_bus() -> GF_EventBus: return _services.get(KEY_EVENT_BUS, null) as GF_EventBus
func get_log() -> GF_LogService: return _services.get(KEY_LOG, null) as GF_LogService
func get_ecs_world() -> GF_EcsWorld: return _services.get(KEY_ECS_WORLD, null) as GF_EcsWorld
func get_ecs_scheduler() -> GF_EcsScheduler: return _services.get(KEY_ECS_SCHEDULER, null) as GF_EcsScheduler
