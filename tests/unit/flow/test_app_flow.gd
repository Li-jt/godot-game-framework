...
var _flow: GF_AppFlow


func before_each() -> void:
	_flow = GF_AppFlow.new()
...
func test_transition_to_valid_state() -> void:
	...
	var result := _flow.transition_to(GF_AppFlow.STATE_MAIN_MENU)
	assert_true(result.is_ok())
	assert_eq(_flow.get_state(), GF_AppFlow.STATE_MAIN_MENU)
...
func test_register_state_adds_to_table() -> void:
	_flow.register_state(&"playing", [GF_AppFlow.STATE_MAIN_MENU], [GF_AppFlow.STATE_PAUSE])
...
func test_initial_state_is_boot() -> void:
	assert_eq(_flow.get_state(), GF_AppFlow.STATE_BOOT)
