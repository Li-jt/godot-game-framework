...
func test_created_stores_data() -> void:
	var data := [1, 2, 3]
	var r := OperationResult.created(data)
	assert_eq(r.data, data)
...
func test_fail_sets_correct_code() -> void:
	var r := OperationResult.fail(OperationResult.ERR_NOT_FOUND, "实体不存在", "TestModule")
	assert_eq(r.status_code, 404)


func test_fail_creates_error_info() -> void:
	var r := OperationResult.fail(OperationResult.ERR_VALIDATION, "校验失败", "TestModule")
...
func test_fail_sets_source_module() -> void:
	var r := OperationResult.fail(OperationResult.ERR_INTERNAL, "内部错误", "MyModule")
	assert_eq(r.error.source_module, "MyModule")


func test_fail_is_not_ok() -> void:
	var r := OperationResult.fail(OperationResult.ERR_NOT_FOUND, "not found")
	assert_false(r.is_ok())
	assert_true(r.is_fail())


func test_fail_empty_source_module() -> void:
	var r := OperationResult.fail(OperationResult.ERR_IO, "IO 错误")
...
func test_is_ok_and_is_fail_opposite() -> void:
	...
	var fail := OperationResult.fail(OperationResult.ERR_NOT_FOUND, "not found")
...
func test_with_context_appends_to_error() -> void:
	var r := OperationResult.fail(OperationResult.ERR_BAD_REQUEST, "请求无效", "TestModule")
	r.with_context("entity_id", 42)
	assert_eq(r.error.context["entity_id"], 42)


func test_with_context_chainable() -> void:
	var r := OperationResult.fail(OperationResult.ERR_BAD_REQUEST, "bad")
...
func test_wrap_preserves_status_and_sets_cause() -> void:
	var original := OperationResult.fail(OperationResult.ERR_NOT_FOUND, "原始错误", "OriginalModule")
	var wrapped := OperationResult.wrap(original, "WrapperModule", "包装后的错误")
...
func test_root_cause_traverses_nested() -> void:
	var a := OperationResult.fail(OperationResult.ERR_IO, "最底层错误", "A")
	var b := OperationResult.wrap(a, "B", "中层包装")
	var c := OperationResult.wrap(b, "C", "顶层包装")
...
func test_root_cause_returns_single_error() -> void:
	var r := OperationResult.fail(OperationResult.ERR_INTERNAL, "单层错误", "X")
...
func test_status_text_for_all_codes(param: Array = use_parameters(status_code_params)) -> void:
	...
	var r := OperationResult.fail(code, "test")
	assert_eq(r.status_text(), expected)


func test_status_text_for_unknown_code() -> void:
	var r := OperationResult.fail(999, "ninety nine nine")
