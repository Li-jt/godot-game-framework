...
## 集成测试：存档完整往返流程。
## SaveService + FakeSaveProvider + ISaveable — 端到端存档恢复。
extends GutTest
...
class TestRoundtripSaveable extends ISaveable:
	var _key: String
...
func test_multiple_slots_independent() -> void:
	...
	var meta1 := SaveMeta.new(); meta1.slot_id = 1
	_service.save_all(1, meta1)

	hero._data = {"hp": 50}
	var meta2 := SaveMeta.new(); meta2.slot_id = 2
