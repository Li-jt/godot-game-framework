...
class_name FakeFileSystemService
...
func read_text(p_path: String) -> OperationResult:
	if not _files.has(p_path):
		return OperationResult.fail(OperationResult.ERR_NOT_FOUND, "file not found: %s" % p_path, "FakeFS")
	return OperationResult.ok(_files[p_path])
...
func read_json(p_path: String) -> OperationResult:
	if _jsons.has(p_path):
		return OperationResult.ok(_jsons[p_path])
	if _files.has(p_path):
		var parsed = JSON.parse_string(_files[p_path])
		if parsed is Dictionary:
			return OperationResult.ok(parsed)
		return OperationResult.fail(OperationResult.ERR_IO, "JSON parse error: %s" % p_path, "FakeFS")
	return OperationResult.fail(OperationResult.ERR_NOT_FOUND, "file not found: %s" % p_path, "FakeFS")
