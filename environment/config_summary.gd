...
static func print_to_log(p_config: AppConfig, p_log: LogService) -> void:
	...
	p_log.info("ConfigSummary", "")
	p_log.info("ConfigSummary", "--- 线程 ---")
	p_log.info("ConfigSummary", _kv("Threading Enabled", p_config.threading.enabled))
	p_log.info("ConfigSummary", _kv("Max Active Jobs", p_config.threading.max_active_jobs))
	p_log.info("ConfigSummary", _kv("Dispatch Per Tick", p_config.threading.max_dispatch_per_tick))
	p_log.info("ConfigSummary", _kv("Default Timeout (ms)", p_config.threading.default_timeout_ms))
	p_log.info("ConfigSummary", _kv("Slow Warn (ms)", p_config.threading.slow_job_warn_ms))
	p_log.info("ConfigSummary", _kv("History Limit", p_config.threading.history_limit))
	p_log.info("ConfigSummary", "")
