...
// GF_EcsNativeWorld 实现——Flecs 后端（性能路线图 §1.6 第一步）。
//
...
void GF_EcsNativeWorld::_bind_methods() {
...
GF_EcsNativeWorld::GF_EcsNativeWorld() {
	world = ecs_init();
...
int64_t GF_EcsNativeWorld::spawn() {
...
bool GF_EcsNativeWorld::despawn(int64_t p_entity) {
...
bool GF_EcsNativeWorld::has_entity(int64_t p_entity) {
	return ecs_is_alive(world, static_cast<uint64_t>(p_entity));
}

int64_t GF_EcsNativeWorld::entity_count() {
	return alive_count;
}
...
bool GF_EcsNativeWorld::add_component(int64_t p_entity, int64_t p_type_key, const Variant &p_data) {
	ecs_entity_t e = static_cast<uint64_t>(p_entity);
	if (!ecs_is_alive(world, e)) {
		return false;
	}
...
bool GF_EcsNativeWorld::set_component(int64_t p_entity, int64_t p_type_key, const Variant &p_data) {
	ecs_entity_t e = static_cast<uint64_t>(p_entity);
	if (!ecs_is_alive(world, e)) {
		return false;
	}
...
Variant GF_EcsNativeWorld::get_component(int64_t p_entity, int64_t p_type_key) {
	ecs_entity_t e = static_cast<uint64_t>(p_entity);
	if (!ecs_is_alive(world, e)) {
		return Variant();
	}
...
bool GF_EcsNativeWorld::remove_component(int64_t p_entity, int64_t p_type_key) {
	ecs_entity_t e = static_cast<uint64_t>(p_entity);
	ecs_entity_t *comp = comp_ids.getptr(p_type_key);
...
bool GF_EcsNativeWorld::has_component(int64_t p_entity, int64_t p_type_key) {
	ecs_entity_t e = static_cast<uint64_t>(p_entity);
	ecs_entity_t *comp = comp_ids.getptr(p_type_key);
...
int64_t GF_EcsNativeWorld::get_version() {
	return version;
}

// 所有存活实体（Flecs 原生 id；GDScript 门面负责映射为框架 id）
PackedInt64Array GF_EcsNativeWorld::all_entities() {
...
void GF_EcsNativeWorld::for_each(int64_t p_type_key, const Callable &p_callback) {
...
double GF_EcsNativeWorld::sum_value_field(int64_t p_type_key) {
	ecs_query_t *q = get_or_create_query(p_type_key);
	if (q == nullptr) {
		return 0.0;
	}
...
Array GF_EcsNativeWorld::flush_events() {
	Array out;
	for (const GF_EcsEventRecord &rec : events) {
		Dictionary d;
...
ecs_entity_t GF_EcsNativeWorld::get_or_create_comp(int64_t p_type_key) {
...
ecs_query_t *GF_EcsNativeWorld::get_or_create_query(int64_t p_type_key) {
...
ecs_query_t *GF_EcsNativeWorld::get_or_create_alive_query() {
...
void GF_EcsNativeWorld::on_alive_event(ecs_iter_t *it) {
	GF_EcsNativeWorld *self = static_cast<GF_EcsNativeWorld *>(it->ctx);
	GF_EcsProbeEventKind kind = (it->event == EcsOnAdd) ? GF_EcsProbeEventKind::ENTITY_ADDED : GF_EcsProbeEventKind::ENTITY_REMOVED;
	for (int i = 0; i < it->count; i++) {
		self->events.push_back({ it->entities[i], 0, kind });
	}
}

void GF_EcsNativeWorld::on_comp_event(ecs_iter_t *it) {
	GF_EcsNativeWorld *self = static_cast<GF_EcsNativeWorld *>(it->ctx);
	GF_EcsProbeEventKind kind;
	if (it->event == EcsOnAdd) {
		kind = GF_EcsProbeEventKind::COMPONENT_ADDED;
	} else if (it->event == EcsOnSet) {
		kind = GF_EcsProbeEventKind::COMPONENT_CHANGED;
	} else {
		kind = GF_EcsProbeEventKind::COMPONENT_REMOVED;
	}
...
int64_t GF_EcsNativeWorld::type_key_of(ecs_entity_t p_component) {
